// TrendFollowing v2 walk-forward decision pass.
//
// v2 strategy targets 8-15 trades/month/symbol with WR ~55-65%, RR ~1:1.5.
// Gates relaxed accordingly (frequency-first, expecting ~50+ trades over IS).
...
const REPORT_PATH = path.join(__dirname, 'tf-v2-report.json');
const PROGRESS_PATH = path.join(__dirname, 'tf-v2-progress.log');
...
// v2 gates: medium-frequency profile.
//   IS  (~6.3 mo): trades≥30 (≈5/mo floor), PF≥1.3, monthlyReturn≥2.0%, DD≤25%, WR≥50%
//   OOS (~2.3 mo): trades≥6, PF≥1.1, no min return, WR≥45%, DD≤30%
function gateIs(m) {
...
(async function main() {
  ...
  for (const symbol of SYMBOLS) {
    ...
    try {
      ...
      log(`  IS: trades=${isM.trades}(${isM.perMonth.toFixed(1)}/mo) PF=${isM.pf.toFixed(2)} mRet=${isM.monthlyReturn.toFixed(1)}% WR=${isM.wr.toFixed(1)}% DD=${isM.dd.toFixed(1)}% sharpe=${isM.sharpe.toFixed(2)}`);
