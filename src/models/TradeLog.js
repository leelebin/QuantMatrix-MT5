...
const TradeLog = {
  ...
  async logOpen(trade) {
    return await tradeLogDb.insert({
      ...
      entryReason: trade.entryReason || trade.signalReason || null,
      setupReason: trade.setupReason || null,
      triggerReason: trade.triggerReason || null,
      strategy: trade.strategy,
      confidence: trade.confidence,
      rawConfidence: trade.rawConfidence ?? trade.confidence ?? null,
      ...
      plannedRiskAmount: trade.plannedRiskAmount ?? null,
      targetRMultiple: trade.targetRMultiple ?? null,
      initialSl: trade.initialSl ?? trade.stopLoss ?? null,
      initialTp: trade.initialTp ?? trade.takeProfit ?? null,
      finalSl: trade.finalSl ?? trade.stopLoss ?? null,
      finalTp: trade.finalTp ?? trade.takeProfit ?? null,
      requestedEntryPrice: trade.requestedEntryPrice ?? null,
      spreadAtEntry: trade.spreadAtEntry ?? null,
      slippageEstimate: trade.slippageEstimate ?? null,
      brokerRetcodeOpen: trade.brokerRetcodeOpen ?? null,
      brokerRetcodeClose: trade.brokerRetcodeClose ?? null,
      brokerRetcodeModify: trade.brokerRetcodeModify ?? null,
      exitPlanSnapshot: trade.exitPlanSnapshot || null,
      managementEvents: Array.isArray(trade.managementEvents) ? trade.managementEvents : [],
      ...
      playbookRole: trade.playbookRole || null,
      preferredEntryStyle: trade.preferredEntryStyle || null,
      beStyle: trade.beStyle || null,
      riskWeight: trade.riskWeight ?? null,
      liveBias: trade.liveBias || null,
      symbolPlaybookSnapshot: trade.symbolPlaybookSnapshot || null,
      ...
      symbolCustomName: trade.symbolCustomName || null,
    ...
    return await tradeLogDb.update(
      { positionDbId },
      {
        $set: {
          ...
          fee: closeData.fee || 0,
          grossProfitLoss: closeData.grossProfitLoss ?? null,
          finalSl: closeData.finalSl ?? null,
          finalTp: closeData.finalTp ?? null,
          brokerRetcodeClose: closeData.brokerRetcodeClose ?? null,
          brokerRetcodeModify: closeData.brokerRetcodeModify ?? null,
          positionSnapshot: closeData.positionSnapshot || null,
          holdingTimeMs,
          holdingTime,
          closedAt: closeData.closedAt,
          realizedRMultiple: closeData.realizedRMultiple ?? null,
          targetRMultipleCaptured: closeData.targetRMultipleCaptured ?? null,
        },
      }
    );
  },
