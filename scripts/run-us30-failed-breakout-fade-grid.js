...
function scoreCandidate(results = []) {
  ...
  return round(
    fullNet * 0.8
      + recentNet * 1.15
      + Math.min(fullTrades, 120) * 0.08
      + Math.min(recentTrades, 60) * 0.12
...
function buildCandidateParameters(symbolCustom = {}) {
  ...
  const allowedHourSets = [
    base.allowedUtcHours || [14, 15, 16, 17, 18, 19],
    [14, 15, 16],
    [15, 16, 17, 18],
    [],
  ];
  ...
  for (const targetMode of ['opposite_boundary', 'adaptive']) {
    for (const allowedUtcHours of allowedHourSets) {
      for (const minSignalScore of [50, 55, 62, 70]) {
        for (const minTargetR of [0.3, 0.5, 0.75]) {
          for (const failedBreakoutLookbackBars of [6, 12, 18]) {
            for (const blockStrongHigherTrend of [false, true]) {
              candidates.push({
                ...base,
                targetMode,
                allowedUtcHours,
                ...
                maxHigherTrendAtr: blockStrongHigherTrend ? 1.15 : base.maxHigherTrendAtr,
              });
            }
          }
        }
...
async function fetchCandles(symbol, timeframes, startDate, endDate) {
  ...
  for (const timeframe of uniqueTimeframes) {
    ...
    console.log(`[US30 fade grid] Fetching ${symbol} ${timeframe} limit=${limit}`);
    byTimeframe[timeframe] = normalizeCandles(
      await mt5PaperService.getCandles(symbol, timeframe, fetchStart, limit, fetchEnd)
    );
    console.log(`[US30 fade grid] Fetched ${byTimeframe[timeframe].length} ${symbol} ${timeframe}`);
  }
  ...
  for (let index = 0; index < candidateParameters.length; index += 1) {
    ...
    const candidate = {
      ...
      parameters: {
        ...
        minTargetR: parameters.minTargetR,
        failedBreakoutLookbackBars: parameters.failedBreakoutLookbackBars,
        blockStrongHigherTrend: parameters.blockStrongHigherTrend,
        maxHigherTrendAtr: parameters.maxHigherTrendAtr,
        requireRejectionCandle: parameters.requireRejectionCandle,
        slAtrBuffer: parameters.slAtrBuffer,
      },
      results,
    };
    ...
    const full = results.find((row) => row.label === 'full_window')?.summary || {};
    const recent = results.find((row) => row.label === 'recent_window')?.summary || {};
    console.log(`[US30 fade grid] ${logicName} candidate=${rankInput} ${index + 1}/${candidateParameters.length} score=${score} full trades=${full.trades} net=${full.netPnl} pf=${full.profitFactor} dd=${full.maxDrawdown} cl=${full.maxConsecutiveLosses} recent trades=${recent.trades} net=${recent.netPnl} pf=${recent.profitFactor}`);
  }
...
async function main() {
  ...
  const maxCombos = Math.max(1, Number(getArg('--max-combos', 24)) || 24);
  ...
  const outputPath = getArg('--out')
    ...
    : path.join(OUTPUT_DIR, `us30-failed-breakout-fade-grid-${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
  const windows = [
  ...
  console.log(`[US30 fade grid] Selected ${symbolCustoms.length} SymbolCustoms`);
  await mt5PaperService.connect();

  const reports = [];
  for (const symbolCustom of symbolCustoms) {
    console.log(`[US30 fade grid] Starting ${symbolCustom.symbolCustomName}`);
    reports.push(await runTarget(symbolCustom, windows, maxCombos, entryStride, candidateIndexSet));
  }

  const report = {
    generatedAt: new Date().toISOString(),
    method: 'Read-only parameter grid for US30 failed-opening-range-breakout fade SymbolCustom draft. DB and strategy flags are not mutated.',
    initialBalance: INITIAL_BALANCE,
    windows,
    maxCombos,
    entryStride,
