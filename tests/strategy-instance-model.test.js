...
function createStrategyInstancesDb(records) {
  return {
    ...
    update: jest.fn(async (query, update, options = {}) => {
      const matchedRecords = records.filter((record) => matchesQuery(record, query));
      if (matchedRecords.length === 0 && options.upsert) {
      ...
      if (options.returnUpdatedDocs) {
        return matchedRecords.length <= 1 ? (matchedRecords[0] || null) : matchedRecords;
      }
...
function loadStrategyInstanceModel({ strategyRecords = [], instanceRecords = [] } = {}) {
  ...
  const instances = instanceRecords.map((record) => ({
    ...record,
    parameters: record.parameters ? JSON.parse(JSON.stringify(record.parameters)) : record.parameters,
    newsBlackout: record.newsBlackout ? JSON.parse(JSON.stringify(record.newsBlackout)) : record.newsBlackout,
  }));
...
describe('strategy instance model', () => {
  ...
  test('create stores a deep-cloned record and findByKey returns it', async () => {
    ...
    const newsBlackout = { enabled: true, beforeMinutes: 10, afterMinutes: 20, impactLevels: ['High'] };
    ...
    newsBlackout.beforeMinutes = 999;
    ...
    expect(found.newsBlackout).toEqual({ enabled: true, beforeMinutes: 10, afterMinutes: 20, impactLevels: ['High'] });
    expect(found.createdAt).toBeInstanceOf(Date);
    expect(found.updatedAt).toBeInstanceOf(Date);
  });

  test('upsert updates existing records and seeds new assignments with empty runtime overrides', async () => {
    const { StrategyInstance } = loadStrategyInstanceModel({
      strategyRecords: [
        {
          name: 'Breakout',
  ...
  test('concurrent upserts for the same missing pair only create one record', async () => {
    const { StrategyInstance, records } = loadStrategyInstanceModel({
      strategyRecords: [
        {
          name: 'Breakout',
          parameters: { lookback_period: 20, body_multiplier: 1.2 },
    ...
    await Promise.all([
      StrategyInstance.upsert('Breakout', 'GBPUSD', {
    ...
    expect(records.filter((record) => record._id === 'Breakout:GBPUSD')).toHaveLength(1);
  ...
  test('migrateFromLegacy is idempotent and seeds empty runtime overrides per symbol', async () => {
