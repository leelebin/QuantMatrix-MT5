// Extra-symbols pass: test 12 untested broker symbols on v2 TrendFollowing.
// Same logic as tf-v2-finalize but only the new candidates, then merge results
// with already-passing set (BTCUSD, US30) and re-sync matrix.
...
const NEW_SYMBOLS = [
  ...
  'SPX500', 'XBRUSD',
  'LTCUSD', 'XRPUSD', 'BCHUSD', 'SOLUSD', 'ADAUSD', 'DOGEUSD',
];
const ALREADY_PASSING = ['BTCUSD', 'US30'];
const STRATEGY = 'TrendFollowing';

const REPORT_PATH = path.join(__dirname, 'tf-v2-extras-report.json');
const PROGRESS_PATH = path.join(__dirname, 'tf-v2-extras-progress.log');
...
(async function main() {
  ...
  log('=== TF v2 EXTRAS pass start ===');
  log(`Period IS=${FULL_START}..${FULL_END}  OOS=${OOS_START}..${OOS_END}  balance=$${INITIAL_BALANCE}`);
  log(`Already passing: ${ALREADY_PASSING.join(', ')}`);
  log(`Testing ${NEW_SYMBOLS.length} new candidates`);
  await refreshAccess();
  ...
  for (const symbol of NEW_SYMBOLS) {
    ...
    try {
      ...
      log(`  IS: trades=${isM.trades}(${isM.perMonth.toFixed(1)}/mo) PF=${isM.pf.toFixed(2)} mRet=${isM.monthlyReturn.toFixed(1)}% WR=${isM.wr.toFixed(1)}% DD=${isM.dd.toFixed(1)}%`);
      log(`  IS params: brk=${params.breakout_lookback} pull=${params.pullback_atr_max} rsi=${params.rsi_buy_min} sl=${params.slMultiplier} tp=${params.tpMultiplier}`);
      item.is = { metrics: isM, gate: isG, params };
  ...
  const finalEnabled = [...ALREADY_PASSING, ...newPasses];
  log(`\nSyncing Strategy.symbols → ${finalEnabled.join(', ')}`);
  const synced = await syncStrategySymbols(finalEnabled);
  log(`  sync result: ${synced}`);

  report.finishedAt = new Date().toISOString();
  report.newPasses = newPasses;
  report.allEnabled = finalEnabled;
  report.syncOk = synced;
  fs.writeFileSync(REPORT_PATH, JSON.stringify(report, null, 2));

  log(`\n=== final summary (extras) ===`);
  for (const it of report.items) {
    const tag = it.status === 'pass' ? 'PASS' : (it.status === 'is_fail' ? 'IS_FAIL' : it.status.toUpperCase());
    if (it.is) {
      log(`  ${tag.padEnd(8)} ${it.symbol.padEnd(8)} IS=${it.is.metrics.trades}t/${it.is.metrics.monthlyReturn.toFixed(1)}%mo PF=${it.is.metrics.pf.toFixed(2)} WR=${it.is.metrics.wr.toFixed(1)}% DD=${it.is.metrics.dd.toFixed(1)}%${it.oos ? `  OOS=${it.oos.metrics.trades}t/${it.oos.metrics.monthlyReturn.toFixed(1)}%mo PF=${it.oos.metrics.pf.toFixed(2)} WR=${it.oos.metrics.wr.toFixed(1)}%` : ''}`);
    } else {
      log(`  ${tag.padEnd(8)} ${it.symbol.padEnd(8)} (no metrics) ${it.error || ''}`);
    }
  }
  log(`\nFinal enabled: ${finalEnabled.join(', ')}`);
